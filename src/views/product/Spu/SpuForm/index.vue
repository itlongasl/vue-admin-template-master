    ...
    <div><el-form label-width="80px">
            ...
            <el-form-item label="品牌">
                <el-select placeholder="请选择品牌" v-model="spu.tmId">
                    <el-option :label="item.tmName" :value="item.id" v-for="item in tradeMrakList"
            ...
            <el-form-item label="SPU图片">
                <el-upload action="/dev-api/admin/product/fileUpload" list-type="picture-card"
                    :on-success="handlerSuccess" :file-list="spuImageList" :on-preview="handlePictureCardPreview"
            ...
            <el-form-item label="销售属性">
                <el-select :placeholder="`还有${unSelectSaleAttr.length}未选择`" v-model="attrIdAndAttrName">
                    <el-option :label="unSelec.name" :value="`${unSelec.id}:${unSelec.name}`"
                        v-for="unSelec in unSelectSaleAttr" :key="unSelec.id">
                    </el-option>
                </el-select>
    ...
    methods: {
        ...
        async initSpuData(spu) {
            ...
            if (spuImageResult.code === 200) {
                let newImageList = spuImageResult.data
        ...
        handleInputConfirm(row) {
            ...
            // 判断属性值是否重复
            let result = row.spuSaleAttrValueList.every(item => item.saleAttrValueName != row.inputValue)
            if (!result) return this.$message('属性值不能重复！')
            // 将input隐藏，显示添加按钮
            row.inputVisible = false
            // 将属性id，输入的属性值，属性名称解构出来
            const { baseSaleAttrId, inputValue, saleAttrName } = row
            let newSaleAttrList = { baseSaleAttrId, saleAttrValueName: inputValue, saleAttrName }
            // 添加到属性值数组
